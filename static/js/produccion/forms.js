        ...
        tblOrden = $('#tblOrden').DataTable({
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    orderable: false,
                    render: function (data, type, row) {
                        return '<input type="number" max= "' + row.cantidad_stock + '" min="1" name="cant"  class="form-control form-control-sm input-sm" autocomplete="off" value="' + row.cantidad + '">';
                    }
                },
                {
                    targets: [5],
...
$(function () {
    ...
    $('.btnFormula').on('click', function (request, response) {
        ...
        console.log(unidad);
        if (unidad == 'Litros'){
            unidad_producto= 'MILILITROS';
        }
        else {
            unidad_producto = 'GRAMOS';
        }
        if (orden.items.objecto_producto.unidad_medida != unidad_producto && orden.items.objecto_producto.unidad_medida != unidad){
            show_notify_error('La unidad de medida de la  cantidad teorica no es compatible con la del producto. Verificar. ');
            orden.items.materias=[];
            orden.list();
            return false;
        }
